...
},
{
    ...
    "ansewr choices": ["Merry", "Sam", "Pippin","Frodo"],
    "answer": 1,
...
class Question extends React.Component {
    constructor(props) {
        ...
        this.id = questions[randomNumber]["correct answer"];
        this.answerGuess = this.answerGuess.bind(this);
        this.displayQuestion = questions[randomNumber].question;
        this.firstChoice = questions[randomNumber]["answer choices"][0];
        this.secondChoice = questions[randomNumber]["answer choices"][1];
        this.thirdChoice = questions[randomNumber]["answer choices"][2];
        this.fourthChoice = questions[randomNumber]["answer choices"][3];
        this.rightAnswer = questions[randomNumber]["answer"];
    ...
    answerGuess(trigger, answer) {
        if (parseInt(trigger) === answer){
            console.log("america")
            this.setState({
                correctQuestion: "akuna"
            })
        } else {
            console.log("canada")
            this.setState({
                incorrectQuestion: "akuna"
            })
        }
    ...
    render() {
        return (
            <div>
                ...
                <h4 id="0" onClick={(e) => {this.answerGuess(e.currentTarget.id, this.rightAnswer)}}>{this.firstChoice}</h4>
                <h4 id="1" onClick={(e) => {this.answerGuess(e.currentTarget.id, this.rightAnswer)}}>{this.secondChoice}</h4>
                <h4 id="2" onClick={(e) => {this.answerGuess(e.currentTarget.id, this.rightAnswer)}}>{this.thirdChoice}</h4>
                <h4 id="3" onClick={(e) => {this.answerGuess(e.currentTarget.id, this.rightAnswer)}}>{this.fourthChoice}</h4>
